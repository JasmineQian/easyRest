...
import com.example.demo.Core.Abstract.AbstractRestAssuredExecute;
import com.example.demo.Core.Filter.CacheOrAutoFilter;
...
import com.example.demo.Enum.ContentType;
  ...
  protected void RestAssuredExecuteVerification(Response response, ExecutionData executionData) {
    ...
    if (contentType.contains(ContentType.JSON.getValue())) {
      ...
      if (!StrUtil.isEmpty(resultAllStringCheck)
        ...
        System.out.println("验证工作执行完成");
      }
    } else {
      //log.warn("暂不支持返回值非{}的数据格式校验功能", ContentType.JSON.getValue());
      System.out.println("暂不支持返回值非{}的数据格式校验功能"+ ContentType.JSON.getValue());
    }
  }
